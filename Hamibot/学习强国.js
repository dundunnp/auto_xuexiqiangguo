...
main_t = threads.start(function () {
  ...
  function refresh(orientation) {
    ...
    log('休眠:' + 'random_time(delay_time * 2)');
  ...
  function back_track() {
    app.launchApp('学习强国');
    log('休眠:' + 'random_time(delay_time * 3)');
    ...
    while (!id('comm_head_title').exists() && while_count < 5) {
      while_count++;
      back();
      log('休眠:' + 'random_time(delay_time)');
      sleep(random_time(delay_time));
    }
    switch (back_track_flag) {
      case 0:
        // 去中心模块      log('等待:' + 'home_bottom_tab_icon_large');
        ...
        // 去province模块      log('等待:' + 'adnroid.view.ViewGroup');
  ...
  if (!finish_list[2] && !finish_list[0]) {
    ...
    // 获取新的完成情况列表  log('休眠:' + 'random_time(delay_time)');
    sleep(random_time(delay_time));
    var back_track_flag = 2;
    back_track();
    var finish_list = get_finish_list();
  ...
  if (!finish_list[1] || !finish_list[2]) {
    ...
    while (completed_watch_count < 6) {
      ...
      try {
        ...
        log('休眠:' + 'Number(current_video_time.slice(4)) * 1000 + 500');
        sleep(Number(current_video_time.slice(4)) * 1000 + 500);
      } catch (error) {
        // 如果被"即将播放"将读取不到视频的时间长度，此时就sleep 3秒      log('休眠:' + '3000');
        sleep(3000);
      }
   ...
   */
  function restart() {
    // 点击退出  log('休眠:' + 'random_time(delay_time)');
    sleep(random_time(delay_time));
    back();
    ...
    while (!flag) {
      ...
      for (num; num < number; num++) {
        ...
        if (textContains('多选题').exists() && is_select_all_choice()) {
          ...
          // 打开查看提示的时间        log('休眠:' + 'random_time(delay_time)');
          sleep(random_time(delay_time));
  ...
  if (!finish_list[5] && special_answer_scored < 8) {
    ...
    if (!className('android.view.View').depth(21).text('学习积分').exists()) back_track();
    entry_model(9);
    // 等待列表加载  log('等待:' + 'android.view.View');
    className('android.view.View').clickable(true).depth(23).waitFor();
    // 打开第一个出现未完成作答的题目
    // 第一个未完成作答的索引
    var special_i = 0;
    ...
    if (text('开始答题').exists() || text('您已经看到了我的底线').exists()) {
      ...
      // 等待题目加载    log('休眠:' + 'random_time(delay_time)');
      sleep(random_time(delay_time));
      // 已完成题数
    ...
    if (is_answer_special_flag) {
      // 点击完成    log('休眠:' + 'random_time(delay_time)');
      sleep(random_time(delay_time));
      log('等待:' + '完成');
      text('完成').waitFor();
      log('点击:' + '完成');
  ...
  if (!finish_list[6]) {
    ...
    while (!flag) {
      ...
      var num = 0;
      while (num < 5) {
        // 每题的过渡      log('休眠:' + 'random_time(delay_time * 2)');
        sleep(random_time(delay_time * 2));
        // 如果答错，第一次通过分享复活
        if (text('分享就能复活').exists()) {
          ...
          sleep(random_time(delay_time / 2));
          back();
          // 等待题目加载        log('休眠:' + 'random_time(delay_time * 3)');
          sleep(random_time(delay_time * 3));
        }
        // 第二次重新开局
        if (text('再来一局').exists()) {
        ...
        // 选项文字列表
        var options_text = [];
        // 等待选项加载      log('等待:' + 'android.widget.RadioButton');
    ...
    // 随意点击直到退出
    do {
      log('休眠:' + 'random_time(delay_time * 2.5)');
  ...
  function do_contest() {
    ...
    while (!text('继续挑战').exists()) {
      // 等待下一题题目加载    log('等待:' + 'android.view.View');
  ...
  if (!finish_list[9] && whether_complete_subscription == 'yes') {
    ...
    if (!className('android.view.View').depth(21).text('学习积分').exists()) back_track();
    entry_model(13);
    // 等待加载  log('休眠:' + 'random_time(delay_time * 3)');
    sleep(random_time(delay_time * 3));
  ...
  if (!finish_list[10]) {
    ...
    if (!className('android.view.View').depth(21).text('学习积分').exists()) back_track();
    entry_model(15);
    // 随意找一篇文章  log('休眠:' + 'random_time(delay_time)');
    sleep(random_time(delay_time));
...
// 启动守护线程,每三秒获取一次FrameLayout，如果PackageName不是学习强国，则结束main_t并重启之
deamon_t = threads.start(function () {
